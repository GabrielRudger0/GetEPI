...
public interface EmprestimoRepository extends JpaRepository<EmprestimoModel, Long> {
    ...
    @Query(value = "SELECT * FROM emprestimo WHERE emprestimo_devolucao IS NULL AND registro_interno = 0", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(*) FROM emprestimo WHERE epi_id_epi = ? AND registro_interno = 0", nativeQuery = true)
    public Integer quantidadeEmprestimoEPI(Long id);

    @Query(value = "SELECT COUNT(*) FROM emprestimo WHERE epi_id_epi = ? AND emprestimo_emissao_data > ? AND emprestimo_emissao_data < ? AND registro_interno = 0", nativeQuery = true)
    public Integer quantidadeEmprestimoEPIPorMes(Long id, String dataInicio, String dataFim);

    @Query(value = "SELECT COUNT(*) FROM emprestimo WHERE emprestimo_emissao_data > ? AND emprestimo_emissao_data < ? AND registro_interno = 0", nativeQuery = true)
    public Integer quantidadeEmprestimoPorDia(String dataInicio, String dataFim);

    @Query(value = "SELECT COUNT(*) FROM emprestimo WHERE emprestimo_devolucao > ? AND emprestimo_devolucao < ? AND registro_interno = 0", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM emprestimo WHERE epi_id_epi = ? AND registro_interno = 0", nativeQuery = true)
